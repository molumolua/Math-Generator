...
def main():
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        ...
        for data in data_list:
                problem=data
                problems[problem['correct_num']].append({
                     "problem":data['complex_problem'],
                     "solution":data['complex_solution']
                })
    # for i in range(11):
    #     result_path=f"./outputs/level-test/openr1_7b_correct_num_{i}.json"
    #     with open(output_path, 'w', encoding='utf-8') as output_json:
    #         json.dump(problems, output_json, ensure_ascii=False, indent=4)
        # process_train_data(problems[i],output_path=result_path,prompt_type="qwen_math",sections=['complex_problem','complex_solution'])
    
    total_problems=sum([problems[10]],[])
    result_path=f"/data2/xucaijun/Math-Generator/outputs/openr1_glm_10.json"
    with open(result_path, 'w', encoding='utf-8') as output_json:
            json.dump(total_problems, output_json, ensure_ascii=False, indent=4)
    # process_train_data(data_list,output_path=result_path,prompt_type="qwen_math",sections=['problem','solution'])
